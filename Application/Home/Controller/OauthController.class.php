...
class OauthController  {
    ...
    public function callback($type = "Weixin", $code = null) {
        (empty($type)) && $this->error('参数错误');
        if (empty($code)) {
            redirect(__ROOT__ . "/");
        }
        ...
        $userInfo = json_decode($sns->http('https://api.weixin.qq.com/sns/userinfo',$param),true);
        $loopApi = new LoopApiController();
        $result = $loopApi->createUser($userInfo['unionid'], 1, $userInfo['headimgurl'], base64_encode($userInfo['nickname']), $userInfo['sex']);
    ...
    private function _bang_handle($user_info, $type, $token) {
        ...
        if ($find_oauth_user) {
            if ($find_oauth_user['uid'] == $current_uid) {
                $this->error("您之前已经绑定过此账号！", U('Center/bang'), 0);
                exit;
            } else {
                $this->error("该帐号已被本站其他账号绑定！", U('Center/bang'), 0);
                exit;
            }
        }
