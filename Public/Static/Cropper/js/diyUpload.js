...
(function ($) {
    $.fn.extend({
        ...
        diyUpload: function (opt, serverCallBack) {
            ...
            webUploader.on('uploadSuccess', function (file, response) {
                ...
                $('.picurl_' + file.id).val(response.picurl);
                $diyBar.fadeOut(1000, function () {
                    $fileBox.children('.diyCancel').show();
                    $fileBox.children('.diySuccess').show();
                    $fileBox.children('.cropper').show();
                    $fileBox.children('.sort-input').show();
                });
                if (successCallBack) {
    ...
    function createBox($fileInput, file, webUploader) {
        ...
        var file_id = file.id;
        $value = parseInt(file_id.split("_", 3)[2]) + 1;
        ...
        var li = '<li id="fileBox_' + file_id + '" class="diyUploadHover"> \
					...
					<div class="diyCancel" onclick=moreImgDel("' + file_id + '")></div> \
					<div class="diySuccess"></div> \
                                        <div class="cropper crop_' + file_id + '" onclick=moreImgCrop("' + file_id + '")>Cropper<input type="hidden" name="picurls[]" value="" class="morepic picurl_' + file_id + '"></div> \
					<div class="sort-input">&nbsp;排序&nbsp;<input type="text" value="' + $value + '" placeholder="sort" name="sorts[]" /></div>\
        ...
        webUploader.makeThumb(file, function (error, dataSrc) {
            if (!error) {
                $fileBox.find('.viewThumb').append('<img class="img-many imgMany_' + file_id + '" src="' + dataSrc + '" >');
            }
        });
    }
