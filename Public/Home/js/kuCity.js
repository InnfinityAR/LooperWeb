...
(function($) {
    ...
    (function() {
        for (var i = 0, len = allCities.length; i < len; i++) {
            ...
            city[ltPart][first] ? city[ltPart][first].push(en) : (city[ltPart][first] = [], city[ltPart][first].push(en+letter));
    ...
    KuCity.prototype = {
        ...
        creatItem: function() {
            ...
            var template = '<div class="kucity"><div class="citybox"><h3 class="kucity_header">请输入城市名称进行检索</h3></div><ul class="result"></ul></div>';
    ...
    $.fn.kuCity = function(options) {
        ...
        target.on('focus', function(e) {
            ...
            kucity.resultct.on('click', 'li', function() {
                kucity.target.val($(this).find('.name').text()+'·'+$(this).find('.letter').text().toUpperCase());
